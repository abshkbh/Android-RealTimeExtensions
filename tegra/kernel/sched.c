...
static void __sched_fork(struct task_struct *p)
{
    ...
    /* Our modification - initializing the
    ...
    //sets it to false
    spin_lock_init(&p->task_spin_lock);
...
need_resched:
	...
	//Getting the current time in order to compute exec time of prev
	//  task being swapped out
	if ((prev->is_budget_set == 1) || (prev->is_log_enabled == 1)) {
	    leader = prev->group_leader;
	    getrawmonotonic( &(t1) );
	}

	if (prev->is_budget_set == 1) {

	    leader = prev->group_leader;
	    //Spinlocking over prev task
	    spin_lock_irqsave(&(leader->task_spin_lock),flagone);
	    //Getting the difference of current time and swap in time for prev task
	    diff = timespec_sub (t1 , leader->exec_time);
	    //Trying to cancel previous budget timer
	    if(hrtimer_try_to_cancel(&(leader->budget_timer)) == -1){ 
		printk("Budget timer for %u in callback\n", prev->pid);
	    }
	    //Adding the difference to compute time
	    leader->compute_time = timespec_add(leader->compute_time , diff);
	    spin_unlock_irqrestore(&(leader->task_spin_lock),flagone);

	}

	if(prev->is_log_enabled == 1 && leader->buf_offset != 0){
	    ...
	    log_data_point(leader, diff);
	    spin_unlock_irqrestore(&(leader->task_spin_lock),flagone);
	}


	//Setting exectimestamp for task before it starts RUNNING
	if ((next->is_budget_set == 1) || (next->is_log_enabled == 1)) {
	    leader = next->group_leader;
	    spin_lock_irqsave(&(leader->task_spin_lock),flagone);
	    getrawmonotonic( &(leader->exec_time) );
	    spin_unlock_irqrestore(&(leader->task_spin_lock),flagone);
	}
	...
	if (next->is_budget_set == 1) {
	    ...
	    if (timespec_compare(&(leader->budget_time) , &(leader->compute_time))> 0) {
